 ...
 * Pledge      : "I pledge my honor that I have abided by the Stevens Honor System." - Daniel Shapiro and Gary Ung
 ******************************************************************************/
#ifndef RBTREE_H_
#define RBTREE_H_
...
public:
    ...
    /**
     * Preincrement operator. Moves forward to next larger .
     */
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x != NULL){
        	if (key == x->key()) {
        		...
        		ss << key;
        		str = ss.str();
        		throw tree_exception("Attempt to insert duplicate key '" + str + "'.");
        		return;
        	}
        	y = x;
        	if(key < x->key()){
    ...
    iterator begin() {
        ...
        // build return  using private constructor
...
//    	}
    ...
    int height(Node<K, V> *node) const {
    	if(node == NULL){
    		...
    		return ((height(node->left)<height(node->right))? height(node->right): height(node->left))+ 1;
    	}
    }
